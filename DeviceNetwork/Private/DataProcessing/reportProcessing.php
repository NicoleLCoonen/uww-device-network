	...
	if(is_post_request() && isset($_POST["data-report"])){
		...
		if($reportType === "general"){
			...
			foreach($modelArr as $model){
				...
				// LL = Lenox Lowercount; Lu = Lenox Upper count
				$f = 0;
				$m = 0;
				$t = 0;
				$LL = 0;
				...
				$count = count_by_floor($locationArr, $f, $m, $t, $LL, $Lu);
				$first[$model] = $count[0]; //corresponds to $f
				$main[$model] = $count[1]; //corresponds to $m
				$third[$model] = $count[2]; //corresponds to $t
				$lenoxLower[$model] = $count[3];//corresponds to $LL
				$lenoxUpper[$model] = $count[4];//corresponds to $Lu
			...
			$floors["Lenox Lower Level"] = $lenoxLower;
			$floors["Lenox Upper Level"] = $lenoxUpper;
			...
			$vendorArr = get_vendors();
				 ...
				 foreach($vendorArr as $vendor){
					 ...
					 $lenoxLower[$vendor] = $count[3];//corresponds to $LL
					 $lenoxUpper[$vendor] = $count[4];//corresponds to $Lu
			...
			$result["Lenox Lower Level"] = $count[3];//corresponds to $LL
			$result["Lenox Upper Level"] = $count[4];//corresponds to $Lu
			
			$floors["First Floor"] = $firstFloor;
			$floors["Main Floor"] = $mainFloor;
			$floors["Third Floor"] = $thirdFloor;
			$floors["Lenox Upper"] = $lenoxUpper;
			$floors["Lenox Lower"] = $lenoxLower;
