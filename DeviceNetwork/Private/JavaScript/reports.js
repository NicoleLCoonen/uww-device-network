...
var Handler = {
	...
	fn: {
		...
		//Handles different layouts for different reports.
		//Some are broken down by floor/model/etc. others are tables, some are both
		styleToGo: function(){
		...
		// I love flexboxes, and many of the forms use them to present information.
		// This makes sure the boxes show up in a logical order
		...
		//Presents a recall form that shows available ports dynamically.
		recallDevice: function(){
			$('.recall').on('click',function(){
				...
				if(formHidden.length === 1){
					...
					// generates a list of available ports as radio inputs labeled with their name
					//$('#AportFloor, #LportFloor').attr("disabled", true);
					$('option').onload(function(){
						//$('#AportFloor, #LportFloor').attr("disabled", false);
					...
					//If the device is Office equipment, generates a list of staff members to whom it can be assigned.
